			...
			<table style="font-size:13px; width: 100%;">
				<tr> 
					...
					<td style="padding-bottom:20px; padding-left: 60px;">
						<!--  <button type="button" id="couponbtn" class="btn btn-outline-black btn-sm">쿠폰 조회/적용</button>-->
						
						<select style="padding-left: 5px; width: 220px; height: 40px;" id="couponselect" name="cp_no">		
							<c:if test="${empty mem_cou}"> <option value = "0"> 사용 가능한 쿠폰이 없습니다.</option> </c:if> 
							<c:if test="${not empty mem_cou}"> <option value = "0">선택안함</option> </c:if>
							<c:forEach var="row" items="${mem_cou}" varStatus="vs">
									<c:choose>
						...
						<span style="font-size: 10px; padding-left: 10px; color:gray;">사용 가능한 쿠폰 <strong style="color:black;">${mem_couCNT}</strong>장 / 보유 쿠폰 ${mem_couponTOTALCNT}장 </span>
					</td>
				</tr>
				...
				<tr>
					...
					<td style="padding-left: 60px;"> 
					    ...
					    <button type="button" onclick="alluse()" style="margin-left:15px; height: 40; line-height: 50%; margin-bottom: 10px; 
						border:1px solid #f0f0f0; background-color:#f0f0f0; padding: 5 10px; width:70px; font-size: 11px;">모두 사용</button>
			...
			<div class="stickyC" style="position: sticky; top: 100px; padding-bottom: 100px;">	
				...
				<div style="overflow: auto; height: 170px;">
					...
					<div style="color:white; margin-top: 20px;">
						...
						<span style="float: right; font-size: 18px; line-height: 1;"> <fmt:formatNumber value="${cart_orderPrice.order_price}" pattern="#,###"/> 원</span>			
																					  <c:set var="order_price" value="${cart_orderPrice.order_price}"/>
					</div>
					<div style="color:white; margin-top: 20px;">
					...
					<div style="color:white; margin-top: 20px;">
						<span style="font-size:13px;"> 포인트 사용 </span>
						<span style="float: right; font-size: 18px; line-height: 1;"> - 
					...
					<div style="color:white; margin-top: 20px;">
						...
						<span style="float: right; font-size: 18px; line-height: 1;"> + 
							<span id="d_fee">
								<c:choose>
									<c:when test="${cart_orderPrice.order_price >= 50000}">0</c:when>
					...
					<div style="color:white; margin-top: 20px;">
						...
						<span style="float: right; font-size: 28px; font-weight: bold; color: lightgreen;"> 
							<!--<c:set var="total_price" value="${cart_orderPrice.order_price}"></c:set>-->
							<c:choose>
								<c:when test="${cart_orderPrice.order_price >= 50000}">
									<span id="total_price"> <fmt:formatNumber value="${cart_orderPrice.order_price}" pattern="#,###"/></span> 
									원
									<!--<c:set var="total_price" value="${cart_orderPrice.order_price}"></c:set>-->
								</c:when>
								<c:otherwise>
									<span id="total_price"> <fmt:formatNumber value="${cart_orderPrice.order_price + 3000}" pattern="#,###"/></span> 
									원
									<!--<c:set var="total_price" value="${cart_orderPrice.order_price + 3000}"></c:set>-->
								</c:otherwise>
							</c:choose>
					...
					<button type="button" onclick="orderCheck()" class="btn btn-outline-blackkk btn-sm" style="font-size: 20px; font-weight: 700;"> CHECK OUT </button>
   ...
   //마일리지 사용 
     ...
     function orderCheck() {
      ...
      if ( $('#rec_addr2').val() == '' ){
       ...
       //alert("pt_minus"+$('#Fpt_minus').val());
       //alert("d_fee"+$('#Fd_fee').val());
