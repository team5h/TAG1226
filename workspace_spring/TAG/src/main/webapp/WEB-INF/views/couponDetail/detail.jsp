   ...
   <div class="leftcontent">
      ...
      <div class="stickyC" style="position: sticky; top: 100px; padding-bottom: 100px;">
         <ul style="font-size:13px; list-style-type: none; padding-left:0px;">
            ...
            <br>
            <li class="side_liT" style="color: lightgreen;">My coupon
   ...
   <div class="rightcontent">
      <div class="couponList">
         ...
         <table class="table table-hover" style="text-align: center;">
            <tr class="info" style="border-top: 3px solid black; border-bottom: 0.1px solid black; font-size: 14px;">
               <th>쿠폰이름</th>
               <th>발급일</th>
               <th>유효일</th>
               <th>공연 이름</th>
            ...
            <tr>
               <td>
                  <c:if test="${row.coupon == 'Bc'}">공연 예매 감사 쿠폰 (10%)</c:if>
                  <c:if test="${row.coupon == 'EBc'}">얼리버드 예매 쿠폰 (15%)</c:if>
                  <c:if test="${row.coupon == 'Dc'}">배송비 할인 쿠폰</c:if>
                  <c:if test="${row.coupon == 'Bday'}">생일 축하 쿠폰 (10%)</c:if>
               </td>
               <td>${row.issue_date}</td>
               <td>${row.exp_date}</td>
               <td>
                  <c:if test="${row.c_no == 1}">LIVE IN SEOUL JOHN K</c:if>
                  <c:if test="${row.c_no == 2}">Kehlani Live in Seoul</c:if>
                  <c:if test="${row.c_no == 3}">HARRY STYLES LOVE ON TOUR</c:if>
                  <c:if test="${row.c_no == 4}">PREP LIVE IN SEOUL</c:if>
                  <c:if test="${row.c_no == 5}">Johnny Stimson Live in Seoul</c:if>
               </td>
               <td>
                  <c:if test="${row.cp_stus == 'Y'}">사용불가</c:if>
                  <c:if test="${row.cp_stus == 'N'}">사용가능</c:if>
               </td>
            </tr>
            </c:forEach>
         </table>
            ...
            <c:if test="${requestScope.count > 0}">
               ...
               <c:set var="endPage" value="${requestScope.endPage}" />
                  ...
                  <c:if test="${startPage > 1}">
                     <a href="/couponDetail/detail?pageNum=${startPage-1}">[이전]</a>
                  </c:if>
                  ...
                  <c:forEach var="i" begin="${startPage}" end="${endPage}">
                     <c:choose>
                        ...
                        <c:when test="${pageNum != i}">
                           <a href="/couponDetail/detail?pageNum=${i}">${i}</a>
                        </c:when>
                     </c:choose>
                  </c:forEach>
                  ...
                  <c:if test="${endPage < pageCount}">
                     <a href="/couponDetail/detail?pageNum=${startPage+5}">[다음]</a>
                  </c:if>
            </c:if>
         </div><!-- 페이징 끝 -->
